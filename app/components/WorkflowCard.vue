...
<template>
  ...
  >
    <NuxtLink :to="`/workflows/${workflow.slug}`" class="block p-4 pb-3 focus-ring rounded-t-xl">
      <div class="flex items-start gap-3">
        ...
        <div class="flex-1 min-w-0">
          <div class="t-ui font-medium truncate" style="color: var(--text-primary);">{{ workflow.name }}</div>
    ...
    <div class="flex items-center gap-2 mx-4 pt-3 pb-4" style="border-top: 1px solid var(--border-subtle);">
      ...
      <ClientOnly>
        <span v-if="workflow.lastRunAt" class="t-small ml-2" style="color: var(--text-disabled);">{{ timeAgo(workflow.lastRunAt) }}</span>
      </ClientOnly>
      ...
      <NuxtLink
        ...
        class="t-small ml-2 flex items-center gap-1 focus-ring rounded"
