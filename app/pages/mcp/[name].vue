            ...
            <div v-if="form.transport === 'stdio'">
              ...
              <div class="space-y-2">
                <div v-for="(pair, idx) in form.envPairs" :key="idx" class="flex items-center gap-2 group">
                  ...
                  <button class="p-1.5 rounded-lg opacity-0 group-hover:opacity-100 focus:opacity-100 transition-all hover:bg-error/10 text-error/60 hover:text-error" aria-label="Remove this environment variable" @click="removeEnvRow(idx)">
            ...
            <div v-else>
              ...
              <div class="space-y-2">
                <div v-for="(pair, idx) in form.headerPairs" :key="idx" class="flex items-center gap-2 group">
                  ...
                  <span class="text-secondary opacity-40">:</span>
                  <input v-model="pair.value" type="text" class="field-input flex-1 font-mono t-small" placeholder="Value" />
                  <button class="p-1.5 rounded-lg opacity-0 group-hover:opacity-100 focus:opacity-100 transition-all hover:bg-error/10 text-error/60 hover:text-error" aria-label="Remove this header" @click="removeHeaderRow(idx)">
                    <UIcon name="i-lucide-trash-2" class="size-3.5" />
                  </button>
                </div>
