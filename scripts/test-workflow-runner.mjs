...
// Union of both branches' fs imports: his added mkdirSync, mine appendFileSync.
...
// attested to 33 commits, 17 files and 1083 lines it never touched.
{
  ...
  const dir = join(process.env.AGENT_RUNS_DIR, r.id, 'artifacts')
  const meta = JSON.parse(readFileSync(join(dir, 'meta.json'), 'utf8'))
  // reconcileFix keeps only what git could never have proved and nothing
  // contradicts - the repo name and the PR link - because a run started
  // without a project directory used to lose its PR URL here, leaving the CI
  // poller with nothing to check. What must NOT survive is any self-reported
  // COUNT: commits, files_changed and lines_changed are claims wearing a
  // fact's shape, and this branch is 33 commits ahead of main.
  for (const repo of meta.fix?.repos ?? []) {
    assert.ok(!('commits' in repo),
      'a self-reported commit list must not survive finalize - git proved nothing here')
  }
