...
export interface RunStep {
  ...
  /** Assistant messages observed on the SDK stream so far. Lightweight,
   *  THROTTLED progress telemetry surfaced from callAgent's message loop
   ...
   *  ended in `error_max_turns` - read as "87 of 40" that looks like a broken
   *  budget, and the budget was working correctly. Use this to see that an
   *  agent is still moving, never to judge how close it is to its limit.
   *
   *  Diagnostic telemetry, not provenance: never asserted by runArtifacts.ts's
   *  runnerOwned() and never written into a step's persisted artifact JSON.
   *  Absent - never a fabricated 0 - when the caller reported nothing. */
