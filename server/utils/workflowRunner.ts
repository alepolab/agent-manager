...
export type AgentCaller =
  (agentSlug: string, input: string, projectDir?: string, signal?: AbortSignal, env?: Record<string, string>,
   onProgress?: (p: AgentProgress) => void) => Promise<AgentCallOutput>
...
async function executeNode(l: Live, run: WorkflowRun, id: string, override?: string): Promise<boolean> {
  ...
  try {
    ...
    const raw = await agentCaller(step.agentSlug, input, run.projectDir, ac.signal, userEnv, (progress: AgentProgress) => {
