...
function applyInterrupted(run: WorkflowRun): WorkflowRun {
  ...
  // The owner is gone - but that does not make the run interrupted. A run
  // record has two halves written separately: the per-step results, and the
  // run-level status/endedAt. Run 011edeb8 reached all seven steps and its
  // final publish never landed, leaving status 'running' with endedAt null,
  // so it read back as interrupted - telling the reader it died when it had
  // succeeded. When every step has settled, derive the outcome from the
  // steps, which ARE durable.
