...
- **`ase_lbss` is a container, not the product.** The parent repo tracks one path under `modules/` (`init.gradle`) and git-ignores the other 49, each of which is its own repo under `alepolab`. Cloning the parent gives an agent almost nothing to work on. The registry entry carries the directory → repo map (`products.yaml`, `crm.modules`) and the run header renders it; clone only the modules a ticket touches, into `modules/<dir>`. The full set is about 8G.
- Directory and repo name differ almost everywhere, and not by a rule: mostly `<dir>_lbss`, but `database` is `crm-database_lbss`, and `control-panel`, `crm-artifacts`, `extdb` and `sasktel-customizations` carry no suffix. Read the map; do not derive a name.
- A run wrongly reported `modules/provisioning/provisioning-api/tomcat/conf/ase/templates/provisioning_script/pcrf_service_provisioning.bsh` absent while holding it, and halted the pipeline on that sentence. Every step is now required to run the search and paste the empty result before reporting anything missing.
